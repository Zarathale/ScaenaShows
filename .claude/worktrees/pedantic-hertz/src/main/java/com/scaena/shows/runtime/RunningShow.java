...
public final class RunningShow {
    ...
    private RunningShow(UUID targetId, String showId, ShowMode mode, AudienceMode audienceMode, Location origin, int durationTicks) {
    ...
    public static RunningShow create(ShowManager mgr, ShowDefinition def, Player target, ShowMode mode, AudienceMode audienceMode) {
        int duration = DurationCalculator.calculate(mgr, def);
        RunningShow rs = new RunningShow(target.getUniqueId(), def.id(), mode, audienceMode, target.getLocation().clone(), duration);
